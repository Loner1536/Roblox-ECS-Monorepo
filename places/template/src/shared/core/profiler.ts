...
export default class Profiler {
	public scheduler?: JabbyScheduler;
	...
	public EnsureSystem(name: string, phase?: string): number | undefined {
		...
		const [ok, id] = pcall(() =>
			this.scheduler!.register_system({ name: key, phase, paused: false }),
		);
	...
	public StartMetricsObserver(intervalSec = 2, extra?: () => string): () => void {
		...
		this.metricsConn = RunService.Heartbeat.Connect((dt) => {
			...
			const memStr =
				okMem && typeOf(totalMb) === "number" ? `${math.floor((totalMb as number) + 0.5)}MB` : "?";
